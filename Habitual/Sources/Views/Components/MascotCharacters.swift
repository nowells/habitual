...
private struct DrawKit {
    static func ellipse(
        _ ctx: inout GraphicsContext, xPos: CGFloat, yPos: CGFloat, width: CGFloat, height: CGFloat, color: Color
    ) {
    ...
    static func roundedRect(
        _ ctx: inout GraphicsContext, xPos: CGFloat, yPos: CGFloat, width: CGFloat, height: CGFloat, radius: CGFloat,
        color: Color
    ) {
        let rect = CGRect(x: xPos - width / 2, y: yPos - height / 2, width: width, height: height)
    ...
    static func openMouth(_ ctx: inout GraphicsContext, xPos: CGFloat, yPos: CGFloat, width: CGFloat, height: CGFloat) {
        ellipse(
            &ctx, xPos: xPos, yPos: yPos, width: width, height: height, color: Color(red: 0.2, green: 0.1, blue: 0.1))
        // tongue
        ellipse(
            &ctx, xPos: xPos, yPos: yPos + height * 0.25, width: width * 0.55, height: height * 0.5,
            color: Color(red: 0.9, green: 0.45, blue: 0.45))
    }
    ...
    static func cuteEyes(
        _ ctx: inout GraphicsContext, leftX: CGFloat, rightX: CGFloat, yPos: CGFloat, eyeWidth: CGFloat,
        eyeHeight: CGFloat,
        mood: MascotMood
    ) {
        ...
        default:
            ...
            // Highlight
            circle(
                &ctx, x: leftX - eyeWidth * 0.12, y: yPos - eyeHeight * 0.15, r: eyeWidth * 0.18, color: highlightColor)
            circle(
                &ctx, x: rightX - eyeWidth * 0.12, y: yPos - eyeHeight * 0.15, r: eyeWidth * 0.18, color: highlightColor
            )
        }
    }
    ...
    static func moodMouth(_ ctx: inout GraphicsContext, xPos: CGFloat, yPos: CGFloat, size: CGFloat, mood: MascotMood) {
        ...
        case .happy:
            smile(
                &ctx, centerX: xPos, y: yPos - size * 0.1, width: size * 0.8, height: size * 0.3, color: dark,
                lineWidth: 2.5)
        case .encouraging:
            // Small "o" mouth
        ...
        case .relaxed:
            // Gentle wavy smile
            smile(
                &ctx, centerX: xPos, y: yPos - size * 0.15, width: size * 0.55, height: size * 0.15, color: dark,
                lineWidth: 2
            )
        }
    }
...
enum DragonRenderer {
    static func draw(in ctx: GraphicsContext, rect: CGRect, mood: MascotMood) {
        ...
        // Body
        DrawKit.ellipse(
            &ctx, xPos: cx, yPos: cy + size * 0.05, width: size * 0.52, height: size * 0.48, color: bodyColor)

        // Belly patch
        DrawKit.ellipse(
            &ctx, xPos: cx, yPos: cy + size * 0.1, width: size * 0.3, height: size * 0.28, color: bellyColor)

        // Head
        DrawKit.ellipse(
            &ctx, xPos: cx, yPos: cy - size * 0.16, width: size * 0.44, height: size * 0.38, color: bodyColor)
        ...
        DrawKit.cuteEyes(
            &ctx, leftX: cx - eyeSpacing, rightX: cx + eyeSpacing, yPos: eyeY, eyeWidth: size * 0.08,
            eyeHeight: size * 0.09, mood: mood)
        ...
        // Blush
        DrawKit.blush(
            &ctx, leftX: cx - size * 0.16, rightX: cx + size * 0.16, yPos: cy - size * 0.12, radius: size * 0.04)

        // Feet
        DrawKit.ellipse(
            &ctx, xPos: cx - size * 0.12, yPos: cy + size * 0.3, width: size * 0.13, height: size * 0.07,
            color: darkGreen)
        DrawKit.ellipse(
            &ctx, xPos: cx + size * 0.12, yPos: cy + size * 0.3, width: size * 0.13, height: size * 0.07,
            color: darkGreen)
...
enum CatRenderer {
    ...
    static func draw(in ctx: GraphicsContext, rect: CGRect, mood: MascotMood) {
        ...
        // Body
        DrawKit.ellipse(
            &ctx, xPos: cx, yPos: cy + size * 0.08, width: size * 0.46, height: size * 0.42, color: bodyColor)

        // Belly
        DrawKit.ellipse(
            &ctx, xPos: cx, yPos: cy + size * 0.12, width: size * 0.26, height: size * 0.25, color: bellyColor)

        // Head
        DrawKit.ellipse(
            &ctx, xPos: cx, yPos: cy - size * 0.14, width: size * 0.46, height: size * 0.38, color: bodyColor)
        ...
        // Forehead stripes
        DrawKit.line(
            &ctx, from: CGPoint(x: cx - size * 0.04, y: cy - size * 0.28),
            to: CGPoint(x: cx - size * 0.06, y: cy - size * 0.22),
        ...
        DrawKit.cuteEyes(
            &ctx, leftX: cx - size * 0.1, rightX: cx + size * 0.1, yPos: eyeY, eyeWidth: size * 0.08,
            eyeHeight: size * 0.09, mood: mood)
        ...
        // Blush
        DrawKit.blush(
            &ctx, leftX: cx - size * 0.16, rightX: cx + size * 0.16, yPos: cy - size * 0.06, radius: size * 0.04)

        // Paws
        DrawKit.ellipse(
            &ctx, xPos: cx - size * 0.1, yPos: cy + size * 0.28, width: size * 0.12, height: size * 0.08,
            color: darkOrange)
        DrawKit.ellipse(
            &ctx, xPos: cx + size * 0.1, yPos: cy + size * 0.28, width: size * 0.12, height: size * 0.08,
            color: darkOrange)
...
enum CapybaraRenderer {
    static func draw(in ctx: GraphicsContext, rect: CGRect, mood: MascotMood) {
        ...
        DrawKit.ellipse(
            &ctx, xPos: cx, yPos: cy + size * 0.08, width: size * 0.54, height: size * 0.44, color: bodyColor)

        // Belly
        DrawKit.ellipse(
            &ctx, xPos: cx, yPos: cy + size * 0.14, width: size * 0.32, height: size * 0.26, color: bellyColor)

        // Head — large and round
        DrawKit.ellipse(
            &ctx, xPos: cx, yPos: cy - size * 0.13, width: size * 0.48, height: size * 0.4, color: bodyColor)

        // Snout area — slightly lighter, round bump
        DrawKit.ellipse(
            &ctx, xPos: cx, yPos: cy - size * 0.04, width: size * 0.22, height: size * 0.14, color: bellyColor)

        // Small round ears
        DrawKit.ellipse(
            &ctx, xPos: cx - size * 0.18, yPos: cy - size * 0.3, width: size * 0.1, height: size * 0.08,
            color: bodyColor)
        DrawKit.ellipse(
            &ctx, xPos: cx + size * 0.18, yPos: cy - size * 0.3, width: size * 0.1, height: size * 0.08,
            color: bodyColor)
        // Inner ear
        DrawKit.ellipse(
            &ctx, xPos: cx - size * 0.18, yPos: cy - size * 0.3, width: size * 0.06, height: size * 0.05,
            color: darkBrown)
        DrawKit.ellipse(
            &ctx, xPos: cx + size * 0.18, yPos: cy - size * 0.3, width: size * 0.06, height: size * 0.05,
            color: darkBrown)
        ...
        let eyeY = cy - size * 0.18
        DrawKit.cuteEyes(
            &ctx, leftX: cx - size * 0.1, rightX: cx + size * 0.1, yPos: eyeY, eyeWidth: size * 0.065,
            eyeHeight: size * 0.07, mood: mood)

        // Nose — big oval
        DrawKit.ellipse(
            &ctx, xPos: cx, yPos: cy - size * 0.06, width: size * 0.1, height: size * 0.06, color: noseColor)
        ...
        if mood == .excited || mood == .celebrating {
            ...
            // Buck teeth
            DrawKit.roundedRect(
                &ctx, xPos: cx - size * 0.02, yPos: cy, width: size * 0.03, height: size * 0.04, radius: 1,
                color: .white)
            DrawKit.roundedRect(
                &ctx, xPos: cx + size * 0.02, yPos: cy, width: size * 0.03, height: size * 0.04, radius: 1,
                color: .white)
            ...
            DrawKit.smile(
                &ctx, centerX: cx, y: cy + size * 0.0, width: size * 0.1, height: size * 0.04, color: darkBrown,
                lineWidth: 2)
        } else {
            // Relaxed — very gentle smile
            DrawKit.smile(
                &ctx, centerX: cx, y: cy - size * 0.01, width: size * 0.07, height: size * 0.02, color: darkBrown,
                lineWidth: 1.5
            )
        }
        ...
        // Blush
        DrawKit.blush(
            &ctx, leftX: cx - size * 0.15, rightX: cx + size * 0.15, yPos: cy - size * 0.08, radius: size * 0.04)

        // Tiny stubby legs
        DrawKit.ellipse(
            &ctx, xPos: cx - size * 0.14, yPos: cy + size * 0.3, width: size * 0.11, height: size * 0.07,
            color: darkBrown)
        DrawKit.ellipse(
            &ctx, xPos: cx + size * 0.14, yPos: cy + size * 0.3, width: size * 0.11, height: size * 0.07,
            color: darkBrown)
        ...
        if mood == .relaxed {
            ...
            leafPath.addQuadCurve(
                to: CGPoint(x: cx + size * 0.12, y: cy - size * 0.42),
                control: CGPoint(x: cx + size * 0.12, y: cy - size * 0.32))
            leafPath.addQuadCurve(
                to: CGPoint(x: cx + size * 0.02, y: cy - size * 0.32),
                control: CGPoint(x: cx + size * 0.02, y: cy - size * 0.42))
            ctx.fill(leafPath, with: .color(Color(red: 0.4, green: 0.75, blue: 0.35)))
            // Stem
            DrawKit.line(
                &ctx, from: CGPoint(x: cx + size * 0.02, y: cy - size * 0.32),
                to: CGPoint(x: cx + size * 0.0, y: cy - size * 0.28),
...
enum DogRenderer {
    static func draw(in ctx: GraphicsContext, rect: CGRect, mood: MascotMood) {
        ...
        // Body
        DrawKit.ellipse(
            &ctx, xPos: cx, yPos: cy + size * 0.08, width: size * 0.48, height: size * 0.42, color: bodyColor)

        // Belly
        DrawKit.ellipse(
            &ctx, xPos: cx, yPos: cy + size * 0.12, width: size * 0.28, height: size * 0.25, color: bellyColor)
        ...
        let leftEarPath = earPath(
            cx: cx - size * 0.17, cy: cy - size * 0.22, width: size * 0.12, height: size * 0.2, angle: -15, size: size)
        ctx.fill(leftEarPath, with: .color(darkBrown))
        // Right ear
        let rightEarPath = earPath(
            cx: cx + size * 0.17, cy: cy - size * 0.22, width: size * 0.12, height: size * 0.2, angle: 15, size: size)
        ctx.fill(rightEarPath, with: .color(darkBrown))

        // Face patch — lighter muzzle area
        DrawKit.ellipse(
            &ctx, xPos: cx, yPos: cy - size * 0.06, width: size * 0.22, height: size * 0.16, color: bellyColor)
        ...
        // Nose
        DrawKit.ellipse(
            &ctx, xPos: cx, yPos: cy - size * 0.07, width: size * 0.08, height: size * 0.05, color: noseColor)
        ...
        if mood == .excited || mood == .celebrating || mood == .happy {
            ...
            // Tongue hanging out
            DrawKit.ellipse(
                &ctx, xPos: cx, yPos: cy + size * 0.06, width: size * 0.06, height: size * 0.07, color: tongueColor)
        } else if mood == .encouraging {
            DrawKit.ellipse(
                &ctx, xPos: cx, yPos: cy + size * 0.01, width: size * 0.04, height: size * 0.05, color: noseColor)
        } else {
            DrawKit.smile(
                &ctx, centerX: cx, y: cy - size * 0.0, width: size * 0.08, height: size * 0.03, color: noseColor,
                lineWidth: 2)
        }

        // Blush
        DrawKit.blush(
            &ctx, leftX: cx - size * 0.15, rightX: cx + size * 0.15, yPos: cy - size * 0.07, radius: size * 0.04)

        // Paws
        DrawKit.ellipse(
            &ctx, xPos: cx - size * 0.11, yPos: cy + size * 0.29, width: size * 0.13, height: size * 0.08,
            color: darkBrown)
        DrawKit.ellipse(
            &ctx, xPos: cx + size * 0.11, yPos: cy + size * 0.29, width: size * 0.13, height: size * 0.08,
            color: darkBrown)
        ...
        DrawKit.ellipse(
            &ctx, xPos: cx, yPos: collarY, width: size * 0.32, height: size * 0.04,
            color: Color(red: 0.85, green: 0.25, blue: 0.25))
        // Tag
        DrawKit.circle(
            &ctx, x: cx, y: collarY + size * 0.03, r: size * 0.025, color: Color(red: 0.95, green: 0.8, blue: 0.2))
    }

    private static func earPath(
        cx: CGFloat, cy: CGFloat, width: CGFloat, height: CGFloat, angle: Double, size: CGFloat
    ) -> Path {
        var path = Path()
