...
8. **macOS `Form` two-column layout squishes controls** — On macOS, `Form` defaults to a two-column label+control style. Long TextField placeholder strings become left-column labels, leaving very little width for the actual controls. `SnapshotContainer` applies `.formStyle(.grouped)` on macOS automatically to get iOS-style full-width rows — no action needed in tests.
